...
function log_marginal(X::Array{ℜ,3}, R::Ƶ; μ::ℜ=3.5, γ::ℜ=0.1) where {ℜ<:Real, Ƶ<:Int}
    ...
    a::ℜ, b::ℜ = I*J*K*γ, γ/μ
    # Filling alpha values
    α_R, α_IR, α_JR, α_KR = fill(a/R,R), fill(a/(I*R),I,R), fill(a/(J*R),J,R), fill(a/(K*R),K,R)
    
    # preparing the S matrices
    s, S_R, S_IR, S_JR, S_KR, S⁺ = zeros(ℜ,R), zeros(ℜ,R), zeros(ℜ,I,R), zeros(ℜ,J,R), zeros(ℜ,K,R), sum(X)
    log_PX, log_PS = -Inf, -Inf
    i::Ƶ, j::Ƶ , k::Ƶ = 0, 0, 0
    
    log_C = a*log(b) - (a + S⁺)*log(b + 1.0) #+ loggamma(a + S⁺) - loggamma(a)  
    log_C += 2.0*sum(loggamma.(α_R)) - sum(loggamma.(α_IR)) - sum(loggamma.(α_JR)) - sum(loggamma.(α_KR))
    
    # If R is one, all component matrices are marginals of X
    if R == 1 
        S_R .= S⁺
        ...
        log_PS = log_C
        log_PS += sum(loggamma.(α_IR .+ S_IR)) + sum(loggamma.(α_JR .+ S_JR)) + sum(loggamma.(α_KR .+ S_KR)) - 2.0*sum(loggamma.(α_R .+ S_R))
        log_PS -= sum(loggamma.(X .+ 1.0))
        return log_PS
    end
    ...
    for S_div = product(map(X_ijk -> combinations(1:(X_ijk+R-1),R-1), X_flat)...)
        ...
        for (ijk, part) = enumerate(S_div)
            ...
            S_JR[j,:] .+= s
            S_KR[k,:] .+= s
            log_PS -= sum(loggamma.(s .+ 1.0))
        end
        log_PS += sum(loggamma.(S_IR)) + sum(loggamma.(S_JR)) + sum(loggamma.(S_KR)) - 2.0*sum(loggamma.(S_R)) + log_C
